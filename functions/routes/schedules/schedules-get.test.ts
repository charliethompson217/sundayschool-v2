...
const { mockUser, mockGetWeekMeta, mockGetWeekGames, mockGetGamesByWeek } = vi.hoisted(() => ({
  mockUser: {
    id: 'user-1',
...
import { handler } from './schedules-get';
...
describe('schedules-get handler', () => {
  beforeEach(() => {
  ...
  it('returns 404 when the week META does not exist', async () => {
    ...
    const res = await invoke(makeEvent({ year: '2024', seasonType: '2', week: '99' }));
  ...
  it('returns 404 for non-admin users trying to view an unpublished week', async () => {
    ...
    const res = await invoke(makeEvent({ year: '2024', seasonType: '2', week: '1' }));
    expect(res.statusCode).toBe(404);
  });
  ...
  it('returns 200 with meta and enriched games for an admin', async () => {
    ...
    expect(body(res).meta).toMatchObject({ week: '1', kind: 'regular' });
  ...
  it('sets espn = null for games with no matching ESPN record', async () => {
    mockGetGamesByWeek.mockResolvedValue([]);
    const res = await invoke(makeEvent({ year: '2024', seasonType: '2', week: '1' }));
    expect(res.statusCode).toBe(200);
    const games = body(res).games as Array<Record<string, unknown>>;
  ...
  it('queries both tables in parallel', async () => {
    await invoke(makeEvent({ year: '2024', seasonType: '2', week: '1' }));
