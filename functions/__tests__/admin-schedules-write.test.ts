...
const { mockUser, mockBuildMetaItem, mockBuildGameItem, mockInsertWeek, mockUpdateWeek, mockGetWeekMeta } = vi.hoisted(
  () => ({
    mockUser: {
      id: 'user-1',
...
import { handler as insertHandler } from '../admin-schedules-insert';
import { handler as updateHandler } from '../admin-schedules-update';
...
function makeWriteEvent(params: Record<string, string>, body: unknown): APIGatewayProxyEventV2 {
  return {
...
describe('admin-schedules-insert handler', () => {
  beforeEach(() => {
  ...
  it('returns 403 for non-admin users', async () => {
    ...
    const res = await invoke(insertHandler, makeWriteEvent({ year: '2024', seasonType: '2', week: '1' }, regularBody));
    expect(res.statusCode).toBe(403);
  });

  it('returns 400 when path parameters are missing', async () => {
    const res = await invoke(insertHandler, makeWriteEvent({}, regularBody));
  ...
  it('returns 400 on invalid JSON body', async () => {
    ...
    const res = await invoke(insertHandler, event);
    expect(res.statusCode).toBe(400);
  });

  it('returns 400 when the request body fails Zod validation', async () => {
    const badBody = { meta: { is_published: 'yes' }, games: [] };
    const res = await invoke(insertHandler, makeWriteEvent({ year: '2024', seasonType: '2', week: '1' }, badBody));
    expect(res.statusCode).toBe(400);
  ...
  it('calls buildMetaItem with correct args for regular season', async () => {
    await invoke(insertHandler, makeWriteEvent({ year: '2024', seasonType: '2', week: '1' }, regularBody));
    expect(mockBuildMetaItem).toHaveBeenCalledWith('2024', '2', '1', regularBody.meta);
  });

  it('returns 201 on successful playoff insert', async () => {
    const playoffMeta = { ...builtMeta, kind: 'playoff' as const };
    mockBuildMetaItem.mockReturnValue(playoffMeta);

    const res = await invoke(insertHandler, makeWriteEvent({ year: '2024', seasonType: '3', week: '1' }, playoffBody));
    expect(res.statusCode).toBe(201);
    expect(mockInsertWeek).toHaveBeenCalledOnce();
  });

  it('returns 400 when a playoff body is sent with seasonType 2', async () => {
    const res = await invoke(insertHandler, makeWriteEvent({ year: '2024', seasonType: '2', week: '1' }, playoffBody));
    expect(res.statusCode).toBe(400);
  });

  it('returns 400 when a regular body is sent with seasonType 3', async () => {
    const res = await invoke(insertHandler, makeWriteEvent({ year: '2024', seasonType: '3', week: '1' }, regularBody));
    expect(res.statusCode).toBe(400);
  });
...
describe('admin-schedules-update handler', () => {
  beforeEach(() => {
  ...
  it('returns 403 for non-admin users', async () => {
    mockUser.isAdmin = false;
    const res = await invoke(updateHandler, makeWriteEvent({ year: '2024', seasonType: '2', week: '1' }, regularBody));
    expect(res.statusCode).toBe(403);
  });

  it('returns 400 when path parameters are missing', async () => {
    const res = await invoke(updateHandler, makeWriteEvent({}, regularBody));
    expect(res.statusCode).toBe(400);
  });

  it('returns 400 when the request body fails Zod validation', async () => {
    const badBody = { meta: { is_published: 'not-a-bool' }, games: [] };
    const res = await invoke(updateHandler, makeWriteEvent({ year: '2024', seasonType: '2', week: '1' }, badBody));
    expect(res.statusCode).toBe(400);
    expect(parseBody(res).error).toBe('Validation failed');
  });
  ...
  it('returns 404 when the week does not exist', async () => {
    ...
    const res = await invoke(updateHandler, makeWriteEvent({ year: '2024', seasonType: '2', week: '99' }, regularBody));
  ...
  it('calls getWeekMeta before updating to verify the week exists', async () => {
    await invoke(updateHandler, makeWriteEvent({ year: '2024', seasonType: '2', week: '1' }, regularBody));
  ...
  it('returns 200 on successful playoff update', async () => {
    ...
    const res = await invoke(updateHandler, makeWriteEvent({ year: '2024', seasonType: '3', week: '1' }, playoffBody));
    expect(res.statusCode).toBe(200);
  });
